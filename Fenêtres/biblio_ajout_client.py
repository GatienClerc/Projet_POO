...
from database import Session          # factory SQLAlchemy
from models.client import Client      # modèle ORM Client
...
class biblio_ajout_client(ctk.CTkToplevel):
    ...
    def add_client(self):
        """
        Lit les champs de l'UI et ajoute un Client via SQLAlchemy, sans modifier l'interface.
        Feedback via messagebox standard.
        """
        nom = (self.entry_nom.get() or "").strip()
        id_metier = (self.entry_id.get() or "").strip()  # identifiant interne (optionnel)
        # les autres champs de cette UI (isbn/annee/pages/type/genre/description)
        # ne sont pas "clients", on ignore sauf description :
        description = (self.entry_description.get() or "").strip() or None


        if not nom:
            messagebox.showerror("Ajout du client", "Le nom est obligatoire.")
            return

        session = Session()
        try:
            # (Optionnel) empêcher doublon par identifiant "métier" si ton modèle a un champ unique (ex. code)
            if id_metier and hasattr(Client, "code"):
                exists = session.query(Client).filter(Client.code == id_metier).first()
                if exists:
                    messagebox.showwarning(
                        "Ajout du client",
                        f"Un client avec l'identifiant {id_metier} existe déjà (id={exists.id})."
                    )
                    return

            # Construire les données dynamiquement selon tes colonnes réelles
            data = {}
            ...
            # identifiant "métier" si disponible (code/matricule/reference/identifiant)
            if id_metier:
                for candidate in ("code", "matricule", "reference", "identifiant"):
                    if hasattr(Client, candidate):
                        data[candidate] = id_metier
                        break

            # description / remarques
            for candidate in ("remarques", "notes", "description"):
                if candidate not in data and hasattr(Client, candidate):
                    data[candidate] = description
                    break

            # traçabilité (si champ présent)
            ...
            session.rollback()
            messagebox.showerror("Ajout du client", f"Erreur lors de l'enregistrement : {e}")
            return
        finally:
            session.close()

        messagebox.showinfo("Ajout du client", f"Client ajouté avec succès (id={client.id}).")
        self.destroy()
